...
export default function Stay() {
  ...
  // The valid literal values are spelled out here because this is a real
  // <select> -- setting it to anything other than one of its own option
  // values doesn't error, it just leaves the underlying field state
  // invalid. Confirmed live: the model set this to a human-readable label
  // ("Restock bath towels") instead of the literal option value, which
  // silently submitted the ticket with an empty type and a real 422 from
  // the backend -- one the model never saw, since a ui_click's own
  // PolicyAllowed result only confirms the click was sent, not that the
  // form's own async submission succeeded.
  const ticketTypeSurface = useAgentSurface(
    "stay-help-ticket-type",
    ["focus", "set", "scroll_to"] as AgentAction[],
    `Choose the help request type -- set to exactly one of these literal values: ${GUEST_TICKET_TYPES.map((entry) => `"${entry.value}" (${entry.label})`).join(", ")}`,
  );
  ...
  return (
    <main className="stay-shell">
      <header className="stay-header">
        ...
        <div><span className="stay-kicker">GUEST PORTAL / NOIDA</span><h1>Stay, considered.</h1><p>A useful place for the things that make a stay easier. Select the property you are visiting to begin.</p></div>
